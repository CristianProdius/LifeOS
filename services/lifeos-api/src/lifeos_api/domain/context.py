...
class AreaContextNotFoundError(RuntimeError):
    """Raised when an area context cannot be found for the current user."""


def get_area_context(session: Session, area_slug: str) -> dict[str, Any]:
    ...
    if area is None:
        raise AreaContextNotFoundError("area not found")
