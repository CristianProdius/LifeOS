...
def apply_food_action(
    ...
    if action == "looks_right":
        ...
        return saved_response(food_log_to_dict(food_log))
    if action == "edit_calories":
        ...
        food_log.calories = int(round(numeric_value))
        merge_food_metadata(food_log, metadata)
        return saved_response(food_log_to_dict(food_log))
    ...
    if action == "delete":
        ...
        merge_food_metadata(food_log, metadata)
        return {"status": "deleted", "requires_input": False, "resource": food_log_to_dict(food_log)}
...
def food_needs_input_response(
    ...
    return {"status": "needs_input", "requires_input": True, "resource": food_log_to_dict(food_log)}
...
def apply_workout_action(
    ...
    if action == "start":
        plan.status = "started"
        return saved_response(planned_workout_to_dict(plan))
    if action == "done":
    ...
    if action == "skip":
        ...
        return saved_response(planned_workout_to_dict(plan))
    if action == "change":
        return {"status": "needs_input", "requires_input": True, "resource": planned_workout_to_dict(plan)}
    raise UnsupportedTelegramActionError(action)
...
def apply_task_action(
    ...
    if action == "done":
        if task.status == "done" and task.completed_at is not None:
            return saved_response(task_to_dict(task))
        task.status = "done"
    ...
    if action == "snooze_tomorrow":
        task.status = "todo"
        task.completed_at = None
        task.due_date = action_date + timedelta(days=1)
        return saved_response(task_to_dict(task))
    raise UnsupportedTelegramActionError(action)
...
def apply_habit_action(
    ...
    if action == "done":
        log.value = 1
        return saved_response(habit_log_to_dict(log))
    if action == "missed":
        log.value = 0
        return saved_response(habit_log_to_dict(log))
    if action == "skip":
...
def saved_response(resource: dict[str, Any]) -> dict[str, Any]:
    return {"status": "saved", "requires_input": False, "resource": resource}
