...
def test_food_target_uses_latest_weight_and_default_cut_targets(tmp_path, monkeypatch):
    with make_client(tmp_path, monkeypatch) as client:
...
def test_food_logs_daily_summary_patch_and_delete(tmp_path, monkeypatch):
    with make_client(tmp_path, monkeypatch) as client:
        log_response = client.post(
            "/food/logs",
            json={
...
def test_food_progress_adjusts_only_when_enough_food_and_weight_data(tmp_path, monkeypatch):
    with make_client(tmp_path, monkeypatch) as client:
...
def test_food_context_includes_target_summary_and_progress(tmp_path, monkeypatch):
    with make_client(tmp_path, monkeypatch) as client:
        assert client.post(
            "/food/logs",
            json={
